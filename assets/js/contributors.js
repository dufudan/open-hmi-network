(function () {
  ...
  function renderProfile(c, contributions) {
    ...
    profileRoot.innerHTML = `<section class="page-hero contributor-profile-hero"><div class="container contributor-profile-grid"><div><div class="eyebrow">Contributor Profile</div><div class="contributor-profile-title"><div class="contributor-avatar contributor-avatar-large">${R.esc((c.name || '?').slice(0,1).toUpperCase())}</div><div><h1>${R.esc(c.name)}</h1><p>${R.esc(c.region || '')}</p></div></div><p class="section-copy">${R.esc(c.bio || c.headline || '')}</p><div class="tag-row profile-skills">${skillTags(c.skills)}</div><div class="page-actions">${project}${linkedin}${github}</div></div><aside class="contributor-principle"><div class="mini-label">Capability proof</div><strong>${mine.length} published contribution${mine.length === 1 ? '' : 's'}</strong><p>${c.available_for_projects ? 'This contributor is available for relevant engineering opportunities.' : 'This profile currently represents contribution activity only.'}</p></aside></div></section>
