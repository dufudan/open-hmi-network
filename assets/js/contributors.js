(function () {
  ...
  function contributionCard(item, contributor) {
    ...
    return `<article class="community-contribution-card">
      ...
      <div class="community-actions"><a class="card-link" href="${R.esc(primaryHref)}" ${external ? 'target="_blank" rel="noopener"' : ''}>${action}</a>${item.available_for_projects ? `<a class="card-link muted-link" href="submit-project.html?reference=${encodeURIComponent(item.title)}&contributor=${encodeURIComponent(contributor?.name || item.contributor_id)}">Discuss a Similar Project →</a>` : ''}</div>
    </article>`;
  }
  ...
  function contributorCard(c, contributions) {
    ...
    return `<a class="contributor-card click-card" href="contributor.html?id=${encodeURIComponent(c.id)}">
      ...
      <div class="contributor-stats"><span><strong>${mine.length}</strong> contribution${mine.length === 1 ? '' : 's'}</span><span>${c.available_for_projects ? 'Available via network' : 'Contribution only'}</span></div>
  ...
  function renderProfile(c, contributions) {
    ...
    const linkedin = c.linkedin_url ? `<a class="btn btn-secondary" href="${R.esc(c.linkedin_url)}" target="_blank" rel="noopener">LinkedIn ↗</a>` : '';
    const project = c.available_for_projects ? `<a class="btn btn-primary" href="submit-project.html?contributor=${encodeURIComponent(c.name)}&reference=${encodeURIComponent('Relevant engineering expertise')}">Request Relevant Expertise →</a>` : '';
    profileRoot.innerHTML = `<section class="page-hero contributor-profile-hero"><div class="container contributor-profile-grid"><div><div class="eyebrow">Contributor Profile</div><div class="contributor-profile-title"><div class="contributor-avatar contributor-avatar-large">${R.esc((c.name || '?').slice(0,1).toUpperCase())}</div><div><h1>${R.esc(c.name)}</h1><p>${R.esc(c.region || '')}</p></div></div><p class="section-copy">${R.esc(c.bio || c.headline || '')}</p><div class="tag-row profile-skills">${skillTags(c.skills)}</div><div class="page-actions">${project}${linkedin}${github}</div></div><aside class="contributor-principle"><div class="mini-label">Capability proof</div><strong>${mine.length} published contribution${mine.length === 1 ? '' : 's'}</strong><p>${c.available_for_projects ? 'Relevant project inquiries can be matched through openhmi.network.' : 'This profile currently represents contribution activity only.'}</p></aside></div></section>
