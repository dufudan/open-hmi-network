...
1. Open `https://openhmi.network/contribute.html`.
...
## Advanced path: GitHub / Registry

Technical contributors who prefer a structured workflow can submit through GitHub or open a Pull Request:

1. Copy `data/registry/schema/contributor-template.json` to `data/registry/contributors/<id>.json` if a contributor profile does not already exist.
2. Copy `data/registry/schema/contribution-template.json` to `data/registry/contributions/<id>.json`.
3. Add the relevant paths to `data/registry/index.json`.
